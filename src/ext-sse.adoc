...
. Save interrupted state of supervisor mode
  ...
  .. Set `INTERRUPTED_FLAGS` attribute as follows:
     ...
     ... `INTERRUPTED_FLAGS[XLEN-1:3]` = zero
  .. Set `INTERRUPTED_A6` attribute = interrupted `A6` GPR value
  .. Set `INTERRUPTED_A7` attribute = interrupted `A7` GPR value
. Redirect execution to supervisor event handler
  .. Set `A6` GPR = `ENTRY_ARG` event attribute value
  .. Set `A7` GPR = `INTERRUPTED_PC` event attribute value
